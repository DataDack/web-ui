...
function createMotionElement(tag: string) {
	function MotionElement({ children, ...domProps }: Readonly<MotionProps>) {
		for (const key of motionOnlyProps) delete domProps[key]
...
void mock.module("@/services/api/client", () => ({
	...
	apiDelete: mock((url: string) => {
		...
		return Promise.resolve({ account_id: ACCOUNT_ID, status: "deleting", cleanup_started: true })
	}),
	...
	return render(
		<QueryClientProvider client={queryClient}>
			<MemoryRouter initialEntries={[`/admin/accounts/${ACCOUNT_ID}/resources`]}>
				<Routes>
					<Route
						path="/admin/accounts/:accountId/resources"
						element={<AccountResourcesPage />}
					/>
				</Routes>
			</MemoryRouter>
		</QueryClientProvider>
...
test("delete account confirmation calls the super-admin teardown API and refreshes views", async () => {
	...
	await waitFor(() => {
		...
		expect(
			getCalls.filter((url) => url === `/resources/search/accounts/${ACCOUNT_ID}/resources`)
				.length
		).toBeGreaterThan(1)
		expect(
