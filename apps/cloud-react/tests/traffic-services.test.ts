...
const service = (
  key: string,
  path: string,
  status: CatalogService["status"] = "operational",
): CatalogService => ({
  id: key.length,
  key,
  path,
...
const routes = [
  ...
  "/compute/load-balancers/123",
  "/compute/target-groups/123",
]
...
describe("combined Load Balancer & API Gateway service", () => {
  test("one admin record controls both features independently of compute and networking", () => {
    const catalog = [
      ...
      service("vpc", "/networking", "maintenance"),
      traffic,
    ]
  ...
  test("maintenance closes both features together", () => {
    const catalog = [
      ...
      { ...traffic, status: "maintenance" as const },
    ]
  ...
  test("coming soon applies to both features", () => {
    for (const path of routes)
      expect(gateForPath(path, [{ ...traffic, state: "coming_soon" }])).toBe("maintenance")
  })
  test("all pages share their own sidebar with both feature sections", () => {
    ...
    expect(sidebar?.items.map((item) => item.path)).toEqual([
      "/api-gateway",
      "/compute/load-balancers",
    ])
    expect(
      findServiceByPath("/compute/instances")?.items.some(
        (item) => item.path === "/compute/load-balancers",
      ),
    ).toBe(false)
    expect(
      findServiceByPath("/networking/subnets")?.items.some((item) => item.path === "/api-gateway"),
    ).toBe(false)
  })
})
