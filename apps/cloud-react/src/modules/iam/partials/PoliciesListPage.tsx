...
export function PoliciesListPage() {
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        icon={FileText}
        breadcrumbs={[{ label: t("console.nav.groups.iam") }, { label: t("iam.policies.title") }]}
        ...
        actions={
          <>
            ...
            <Button className="gap-2" onClick={() => void navigate(IAM_ROUTES.POLICY_NEW)}>
      ...
      <Tabs defaultValue="organization" className="space-y-4">
        <TabsList>
          <TabsTrigger value="organization">
            ...
            <span className="ml-1.5 text-[11px] text-muted-foreground">{organization.length}</span>
          </TabsTrigger>
          <TabsTrigger value="system" className="gap-1.5">
            <Lock className="size-3" />
            {t("iam.policies.tabs.system")}
            <span className="ml-1.5 text-[11px] text-muted-foreground">{system.length}</span>
          </TabsTrigger>
        </TabsList>
