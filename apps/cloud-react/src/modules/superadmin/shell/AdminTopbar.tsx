...
export function AdminTopbar({ onOpenMobileNav }: Readonly<{ onOpenMobileNav: () => void }>) {
  ...
  return (
    <header className="sticky top-0 z-40 flex h-13 flex-none items-center gap-3 border-b border-border bg-(--topbar) px-3.5 backdrop-blur-xl">
      <Button
        variant="ghost"
        size="icon"
      ...
      >
        <Logo iconClassName="size-6" className="text-[15px]" wordmarkClassName="hidden sm:inline" />
        <span className="rounded-full border border-brand-gold/40 bg-brand-gold-soft px-2 py-0.5 font-mono text-[10px] font-semibold uppercase tracking-wider text-brand-gold">
          {t("superAdmin.title")}
        </span>
      </Link>
