...
export function StoragePriceFormSheet({ open, onOpenChange, price }: Readonly<Props>) {
	...
	useEffect(() => {
		if (open) {
			reset(
				price
					? {
							availability_zone_id: String(price.availability_zone_id),
							sku: price.sku,
							name: price.name,
	...
	return (
		<FormSheet
			...
			title={
				isEdit
					? t("superAdmin.storagePrices.editTitle")
					: t("superAdmin.storagePrices.createTitle")
			}
		...
		>
					...
					render={({ field }) => (
						...
						>
							<SelectTrigger className="w-full">
								<SelectValue
									placeholder={t(
										"superAdmin.storagePrices.fields.availabilityZonePlaceholder"
									)}
								/>
							</SelectTrigger>
							<SelectContent>
			...
			<Field label="SKU" error={errors.sku?.message}>
				<Input
					{...register("sku")}
					placeholder="blk-ssd-zonal-in1a"
					className="font-mono"
				/>
			</Field>
			...
			<Field
				label="Included throughput MBps"
			...
			>
				<Input
					...
					min={0}
					step="any"
					{...register("price_per_throughput_mbps")}
				/>
			</Field>
			<Field
				label="Snapshot price / GB month"
			...
			>
				<Input
					...
					min={0}
					step="any"
					{...register("snapshot_price_per_gb_month")}
				/>
			</Field>
			...
			<Field
				label={t("superAdmin.storagePrices.fields.currency")}
