...
import { Pencil, Plus, RefreshCw, Server, Cpu, Monitor, HardDrive, Globe, Trash2 } from "lucide-react"
...
export function PVENodesPage() {
    ...
    const columns = useMemo<ColumnDef<PVENode>[]>(
        () => [
            {
                id: "name",
                header: () => <span className="text-xs font-semibold uppercase tracking-wider">{t("superAdmin.pveNodes.fields.name")}</span>,
                accessorFn: (n) => n.name,
                cell: ({ row }) => (
            ...
            },
            {
                ...
                header: () => <span className="text-xs font-semibold uppercase tracking-wider">{t("superAdmin.pveNodes.fields.availabilityZone")}</span>,
            ...
            {
                id: "cpu",
                header: () => <span className="text-xs font-semibold uppercase tracking-wider"><Cpu className="size-3.5 inline mr-1"/>{t("superAdmin.pveNodes.fields.cpu")}</span>,
                accessorFn: (n) => n.cpu_used,
                cell: ({ row }) => {
                    const { cpu_used, cpu_total } = row.original
                    const percentage = cpu_total > 0 ? (cpu_used / cpu_total) * 100 : 0
                    const isHigh = percentage > 85
                    return (
                        <div className="flex flex-col gap-1 w-24">
                            <div className="flex justify-between items-center text-[11px] font-mono">
                                <span>{cpu_used}</span>
                                <span className="text-muted-foreground">/ {cpu_total}</span>
                            </div>
                            <div className="h-1.5 w-full bg-accent/30 rounded-full overflow-hidden">
                                <div className={`h-full ${isHigh ? 'bg-destructive/80' : 'bg-status-info/70'}`} style={{ width: `${percentage}%` }} />
                            </div>
                        </div>
                    )
                },
            ...
            {
                id: "ram",
                header: () => <span className="text-xs font-semibold uppercase tracking-wider"><Monitor className="size-3.5 inline mr-1"/>{t("superAdmin.pveNodes.fields.ram")}</span>,
                accessorFn: (n) => n.ram_used_mb,
                cell: ({ row }) => {
                    const { ram_used_mb, ram_total_mb } = row.original
                    const percentage = ram_total_mb > 0 ? (ram_used_mb / ram_total_mb) * 100 : 0
                    const isHigh = percentage > 85
                    return (
                        <div className="flex flex-col gap-1 w-24">
                            <div className="flex justify-between items-center text-[11px] font-mono">
                                <span>{(ram_used_mb/1024).toFixed(1)}GB</span>
                                <span className="text-muted-foreground">/ {(ram_total_mb/1024).toFixed(1)}GB</span>
                            </div>
                            <div className="h-1.5 w-full bg-accent/30 rounded-full overflow-hidden">
                                <div className={`h-full ${isHigh ? 'bg-destructive/80' : 'bg-status-success/70'}`} style={{ width: `${percentage}%` }} />
                            </div>
                        </div>
                    )
                },
            ...
            {
                id: "storage",
                header: () => <span className="text-xs font-semibold uppercase tracking-wider"><HardDrive className="size-3.5 inline mr-1"/>{t("superAdmin.pveNodes.fields.storage")}</span>,
                accessorFn: (n) => n.storage_used_gb,
                cell: ({ row }) => {
                    const { storage_used_gb, storage_total_gb } = row.original
                    const percentage = storage_total_gb > 0 ? (storage_used_gb / storage_total_gb) * 100 : 0
                    const isHigh = percentage > 85
                    return (
                        <div className="flex flex-col gap-1 w-24">
                            <div className="flex justify-between items-center text-[11px] font-mono">
                                <span>{storage_used_gb}GB</span>
                                <span className="text-muted-foreground">/ {storage_total_gb}GB</span>
                            </div>
                            <div className="h-1.5 w-full bg-accent/30 rounded-full overflow-hidden">
                                <div className={`h-full ${isHigh ? 'bg-destructive/80' : 'bg-brand-gold/70'}`} style={{ width: `${percentage}%` }} />
                            </div>
                        </div>
                    )
                },
            ...
            actionsColumn<PVENode>({
                ...
                actions: () => [
                    { label: t("superAdmin.actions.edit"), icon: Pencil, onAction: openEdit },
                    {
                        ...
                        icon: Trash2,
                        destructive: true,
                        onAction: (node: PVENode) => setDeleting(node),
                    },
                ],
            }),
        ],
        ...
        <div className="space-y-5">
            <PageHeader
                ...
                breadcrumbs={[
                    ...
                    { label: t("superAdmin.pveNodes.title") },
                ]}
                ...
                actions={
                    <>
                        <Button
                            variant="ghost"
                            size="icon"
                            onClick={() => void refreshMetrics.mutate()}
                            disabled={refreshMetrics.isPending || isFetching}
                            aria-label={t("common.refresh")}
                        >
                            <RefreshCw className={`w-4 h-4 ${isFetching || refreshMetrics.isPending ? "animate-spin" : ""}`} />
                        </Button>
