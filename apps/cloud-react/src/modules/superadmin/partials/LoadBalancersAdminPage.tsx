...
function FleetConfiguration({ settings }: Readonly<{ settings: LBSettings }>) {
	...
	const onSubmit = (values: FormValues) => {
		save(values, { onSuccess: (updated) => { reset(settingsToValues(updated)); } })
	}

	const onReset = () => { reset(settingsToValues(settings)); }
...
function NodeManagerRow({ node }: Readonly<{ node: PVENode }>) {
	...
	const onGenerate = () =>
		generate({ id: node.id }, { onSuccess: (creds) => { setIssued(creds); } })
	...
	return (
		<div className="space-y-6">
			...
			{isLoading ? (
				...
				<FleetConfiguration settings={settings} />
			)}
