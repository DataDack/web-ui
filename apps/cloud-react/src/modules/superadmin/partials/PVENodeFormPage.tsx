...
import { CopyButton, CreateWizard, PageHeader, Section, type WizardStep } from "@/components/console"
...
function nodeToValues(node: PVENode): FormValues {
	return {
		availability_zone_id: String(node.availability_zone_id),
		name: node.name,
...
function AgentCredentialsSection({ node }: Readonly<{ node: PVENode }>) {
	...
	const onGenerate = () =>
		generate(
			{ id: node.id },
			{ onSuccess: (creds) => { setIssued(creds); } }
		)
			...
			<div className="space-y-4">
				<div className="grid gap-4 sm:grid-cols-2">
					<div className="space-y-1.5">
						<FieldLabel>
							{t("superAdmin.pveNodes.agentCredentials.clientId")}
						</FieldLabel>
						{clientId ? (
					...
					<div className="space-y-1.5">
						<FieldLabel>
							{t("superAdmin.pveNodes.agentCredentials.secretState")}
						</FieldLabel>
				...
				{issued ? (
					<div className="space-y-3 rounded-md border border-status-warning/40 bg-status-warning/10 p-4">
						...
						<div className="space-y-1.5">
							<FieldLabel>
								{t("superAdmin.pveNodes.agentCredentials.secret")}
							</FieldLabel>
			...
			<div className="space-y-1.5">
				...
				<Controller
					...
					render={({ field }) => {
						...
						const selected = azs.find((a) => String(a.id) === value)
							...
							>
								<SelectTrigger className="w-full">
									{label ? (
										...
										<span className="text-muted-foreground">
											{t(
												"superAdmin.pveNodes.fields.availabilityZonePlaceholder"
											)}
										</span>
									)}
								</SelectTrigger>
