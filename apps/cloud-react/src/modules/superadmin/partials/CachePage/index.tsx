					...
					<div className="space-y-3">
						{groups.map((group) => (
							<ModuleCard
								...
								group={group}
								selection={selection}
								onToggleNamespace={(key) => { setSelection((s) => toggle(s, key)); }}
								onToggleGroup={(g) => { setSelection((s) => toggleGroup(s, g)); }}
			...
			<ConfirmDialog
				...
				description={
					<div className="space-y-2 text-[13px]">
						...
						<p className="text-muted-foreground">
							{t("superAdmin.cache.flush.subtitle")}
						</p>
					</div>
				}
