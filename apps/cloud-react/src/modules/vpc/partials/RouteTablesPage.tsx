  ...
  return (
    <div className="space-y-5">
      <PageHeader
        icon={Route}
        title="Route tables"
        description="Manage VPC routes and the subnets that use them."
        breadcrumbs={[{ label: "Networking" }, { label: "Route tables" }]}
        actions={
          <>
            <Button
              variant="ghost"
              size="icon"
              aria-label="Refresh route tables"
              disabled={tables.isFetching}
              onClick={() => void tables.refetch()}
            >
              <RefreshCw className={`size-4 ${tables.isFetching ? "animate-spin" : ""}`} />
            </Button>
            <Button
              variant="gold"
              onClick={() => {
          ...
          <EmptyState
            ...
            description={
              query
                ...
                : "Create a route table to view local routes and organize subnet associations."
            }
            action={
              query
                ? {
