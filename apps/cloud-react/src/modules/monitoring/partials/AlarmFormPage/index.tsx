...
import { Button, Input, Label, Skeleton, Textarea } from "@datadack/common-ui"
...
export function AlarmFormPage() {
  ...
  const dimensions = useMemo(
    () => dimensionsFor(targetType, firstTargetId, parseDimensions(values.customDimensions) ?? {}),
      ...
      >
        <div className="grid items-start gap-5 lg:grid-cols-[minmax(0,1fr)_340px]">
                  ...
                  <div className="space-y-1.5">
                    ...
                    {values.targetIds.length > 1 && (
                      <p className="text-[11px] text-muted-foreground">
                        Each alarm gets its resource name added, so the {values.targetIds.length}{" "}
                        names stay distinct.
                      </p>
                    )}
          ...
          <SummaryRail
            ...
            metricLine={
              metricLabel ? `${metricLabel} · ${values.statistic}` : "No signal chosen yet"
            }
            conditionLine={ruleSentence(values, metricLabel || "the signal", descriptor.unit)}
            severity={values.severity}
