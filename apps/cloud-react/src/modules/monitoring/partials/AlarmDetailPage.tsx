        ...
        <>
            {!alarm.enabled && (
                <p className="text-[13px] text-status-warning">
                    This alarm is switched off — nothing is being evaluated. Enable it to start
                    checking the metric again.
                </p>
            )}
            ...
            {CHART_RANGES.map((range) => (
                <Button
                    ...
                    className={cn(
                        ...
                        range.id === value &&
                            "border border-brand-gold bg-brand-gold/5 text-brand-gold"
                    )}
                    onClick={() => {
                        onChange(range.id)
                    }}
...
function ChartStats({
    ...
    return (
        <div className="mt-3 flex flex-wrap items-center gap-x-6 gap-y-1">
            ...
            {gapCount > 0 && (
                <span className={META_LINE}>
                    {gapCount} of {bucketCount} periods in the last {rangeLabel} reported nothing —
                    those gaps are left blank, not drawn as zero.
                </span>
            )}
        </div>
    )
            ...
            <Table>
                <TableHeader>
                    <TableRow className="hover:bg-transparent">
                        {["When", "Result", "Channel", "Response", "Reference", "Detail"].map(
                            (header) => (
                    ...
                    {rows.map((row) => {
                        ...
                        return (
                            <TableRow key={row.id}>
                                ...
                                <TableCell className={CELL_MUTED}>
                                    {row.http_status ?? DASH}
                                </TableCell>
                                <TableCell className={CELL}>
                                    <DeliveryReference
            ...
            {!isLoading && rows.length > 0 && (
                <>
                    ...
                    <p className="mt-2 text-[11px] text-muted-foreground">
                        Taken from your account&apos;s recent delivery log, so older attempts for
                        this alarm may have fallen off the end of it.
                    </p>
                </>
            )}
        </Section>
    ...
    return (
        <div className="space-y-5">
            <Section variant="panel" title="Configuration">
                <KeyValueGrid
                    items={configItems(alarm, type, display, unitForAlarm(alarm))}
                    columns={2}
                />
            </Section>
            <Section
