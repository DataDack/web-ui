...
export const monitoringApi = {
  ...
  testSavedChannel: (id: string, payload: { severity?: string } = {}): Promise<TestChannelResult> =>
  ...
  jiraOAuthCallback: (payload: { code: string; state: string }): Promise<JiraConnection[]> =>
    apiPost<JiraConnection[] | null>(`${ALERTS_BASE}/channels/jira/oauth/callback`, payload).then(
      arr,
    ),
  ...
  notifications: (limit = 50): Promise<AlarmNotification[]> =>
    apiGet<AlarmNotification[] | null>(`${ALERTS_BASE}/notifications?limit=${String(limit)}`).then(
      arr,
    ),
