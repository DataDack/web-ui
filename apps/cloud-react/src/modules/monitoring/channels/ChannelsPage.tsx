...
function NameCell({ channel }: Readonly<{ channel: ChannelResponse }>) {
    ...
    return (
        <div className="flex min-w-0 flex-col gap-1">
            ...
            <Badge
                variant="outline"
                className={cn("w-fit gap-1 font-mono text-[10px]", meta.badgeClass)}
            >
    ...
    return (
        <div className="flex items-center gap-2">
            <Switch
                ...
                aria-label={
                    channel.enabled
                        ? `Stop sending to ${channel.name}`
                        : `Start sending to ${channel.name}`
                }
            />
                ...
                { id: channel.id },
                {
                    onSuccess: (testResult) => {
                        if (!testResult.delivered) {
                            toast.error(
                                testResult.error ?? `Test to "${channel.name}" was not delivered`
                            )
                            return
                        }
                        toast.success(
            ...
            },
            {
                ...
                cell: ({ row }) => (
                    <Badge
                        ...
                        className={cn(
                            "font-mono text-[11px]",
                            SEVERITY_BADGE_CLASS[row.original.min_severity]
                        )}
                    >
    ...
    return (
        <div className="space-y-5">
            ...
            <ConfirmDialog
                ...
                description={
                    <>
                        Channel <span className="font-mono">{toDelete?.name}</span> will stop
