...
export function SourceDeploymentPanel({ build, project }: Readonly<SourceDeploymentPanelProps>) {
  ...
  return (
    <aside className="hidden min-h-0 w-72 shrink-0 flex-col border-l border-border/60 bg-muted/10 xl:flex">
          ...
          <div className="mb-2 flex items-center justify-between gap-3">
            ...
            {project && (
              <Button asChild size="icon" variant="ghost" className="size-7">
                <Link
                  to={`${MANAGED_APPS_ROUTES.project(project.id)}?tab=settings`}
