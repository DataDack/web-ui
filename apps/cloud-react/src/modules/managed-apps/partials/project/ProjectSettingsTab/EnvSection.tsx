...
export function EnvSection({ project }: Readonly<{ project: Project }>) {
    ...
    const countLabel =
        names.length === 1 ? "1 variable set" : `${String(names.length)} variables set`
                ...
                description={
                    <span className="block space-y-2">
                        <span className="block">
                            Saving replaces the whole set, and the console was never given the
                            values of variables you have not retyped. These would be saved empty:
                        </span>
                        <span className="block font-mono text-[12px] text-destructive">
                            {wouldClear.join(", ")}
                        </span>
                        <span className="block">
                            Retype a value for any you want to keep, then save again.
                        </span>
                    </span>
                }
            />
