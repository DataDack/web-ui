		...
		<aside className="space-y-4 lg:sticky lg:top-6">
			<div className="glass-1 space-y-3 rounded-xl border border-border/60 p-4">
				...
				<div className="space-y-1.5">
					...
					<Row
						label="Build"
						value={inherited(values.build_command, defaults?.build_command)}
					/>
					<Row
						label="Output"
						value={inherited(values.output_dir, defaults?.output_dir)}
					/>
					<Row
						label="Env vars"
			...
			{issues.length > 0 && (
				<div className="rounded-xl border border-destructive/30 bg-destructive/5 p-3">
					<p className="flex items-center gap-1.5 text-[12px] font-medium text-destructive">
						...
						{issues.length === 1
							? "1 thing to fix"
							: `${String(issues.length)} things to fix`}
					</p>
			...
			<div className="flex items-start gap-2 rounded-lg border border-border/60 px-3 py-2.5">
				...
				<p className="text-[11px] text-muted-foreground">
					We open a pull request on your repository that adds the build workflow. Builds
					run on your own GitHub Actions runners, and nothing builds until you merge it.
				</p>
			</div>
		</aside>
	)
