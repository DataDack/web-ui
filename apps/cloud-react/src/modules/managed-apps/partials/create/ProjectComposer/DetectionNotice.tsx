    ...
    return (
      <div className="flex items-start gap-2 rounded-lg border border-border/60 px-3 py-2.5">
        ...
        <p className="text-[11px] text-muted-foreground">
          No <code className="font-mono">package.json</code> here, so the build settings below are
          platform defaults rather than anything read from your repository.
        ...
        <ul className="mt-2 space-y-1 border-t border-border/40 pt-2">
          ...
          {detection.truncated && (
            <li className="text-[11px] text-status-warning">
              The repository is large enough that GitHub capped the file listing — some directories
              may be missing from the root candidates.
            </li>
          )}
        </ul>
      )}
