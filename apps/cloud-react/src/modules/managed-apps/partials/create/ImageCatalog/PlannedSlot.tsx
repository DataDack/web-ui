...
export function PlannedSlot({ image }: Readonly<PlannedSlotProps>) {
  return (
    <div className="flex flex-col gap-3 rounded-xl border border-dashed border-border/60 p-5 opacity-60">
      <span className="flex size-9 items-center justify-center rounded-lg glass-1-bg-raised text-[11px] font-bold text-muted-foreground">
      ...
      <span className="w-fit rounded-full glass-1-bg-raised px-2 py-0.5 text-[10px] font-semibold tracking-wide text-muted-foreground uppercase">
        Planned
      </span>
    </div>
  )
