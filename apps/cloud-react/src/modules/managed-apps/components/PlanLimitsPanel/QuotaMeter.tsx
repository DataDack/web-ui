...
export function QuotaMeter({ label, used, limit, unit }: Readonly<QuotaMeterProps>) {
  const limitKnown = typeof limit === "number" && Number.isFinite(limit)
  const unlimited = isUnlimited(limit)
  // A limit of 0 has no bar to fill and would divide by zero; it is a real
  // "none", so the ratio is treated as full.
  const ratio = !limitKnown || unlimited || limit <= 0 ? 0 : Math.min(1, used / limit)
  const spent = limitKnown && !unlimited && used >= limit && limit >= 0
  const tight = limitKnown && !unlimited && limit > 0 && ratio >= 0.8
  let barColor = "bg-brand-gold"
  if (spent) barColor = "bg-status-danger"
  else if (tight) barColor = "bg-status-warning"

  let usageText = `${used.toLocaleString()} of ${formatLimit(limit)} ${unit}`
  if (!limitKnown) usageText = `${used.toLocaleString()} ${unit} used · limit unavailable`
  else if (unlimited) usageText = `${used.toLocaleString()} ${unit} · unlimited`

  return (
    <div className="min-w-0">
      <div className="flex items-baseline justify-between gap-3">
        <span className="text-[11px] tracking-wide text-muted-foreground uppercase">{label}</span>
        <span className={cn("font-mono text-[12px] font-medium", spent && "text-status-danger")}>
          {usageText}
        </span>
      </div>
