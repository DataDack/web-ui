...
export function LocationAndNetworkStep({
  ...
  const selectedAzId = useMemo(
    () =>
      regions.find((r) => r.code === region)?.availability_zones.find((az) => az.code === zone)?.id,
    [regions, region, zone],
  )
  const azSubnets = useMemo(
    () =>
      selectedAzId
        ? subnets.filter((s) => !s.availability_zone_id || s.availability_zone_id === selectedAzId)
        : subnets,
          ...
          {regions.map((r) => {
            ...
            return (
              ...
              >
                ...
                <div>
                  <span className="block text-[13px] font-medium text-foreground">{r.name}</span>
              ...
              {zones.map((az) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-[13px] font-medium text-foreground">{az.code}</span>
        ...
        <div className="grid sm:grid-cols-2 gap-3 mb-6">
          <motion.button
            ...
            className={cn(
              ...
              skipVpc ? "gradient-ring bg-accent/20" : "hover:bg-accent/30 border-border-glass",
            )}
          >
              ...
              <>
                <div className="grid sm:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    ...
                    <Select
                      ...
                      onValueChange={(value) => {
                        ...
                        form.setValue("subnet_id", "")
                        form.setValue("_subnet_cidr", "")
                        if (form.getValues("private_ip")) void form.trigger("private_ip")
                      }}
                    >
                    ...
                    <FieldError message={form.formState.errors.vpc_id?.message} />
                  </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    >
                      <SelectTrigger className="w-full">
                        <SelectValue placeholder={t("vms.wizard.subnetPlaceholder")} />
                      </SelectTrigger>
                      <SelectContent>
                        {azSubnets.length === 0 ? (
                    ...
                    <FieldError message={form.formState.errors.subnet_id?.message} />
                  </div>
                </div>
                ...
                <div className="space-y-1.5 max-w-sm">
                  ...
                  <FieldError message={form.formState.errors.private_ip?.message} />
                </div>
              </>
            )}
          </div>
                ...
                <div className="flex items-center justify-between gap-6 max-w-sm">
                  ...
                  <Switch
                    ...
                    onCheckedChange={(checked) => {
                      form.setValue("public_ip_type", checked ? "ephemeral" : "none", {
                        shouldValidate: true,
                      })
                    }}
                  />
...
function SecurityGroupPicker({ form }: Readonly<{ form: UseFormReturn<FormValues> }>) {
  ...
  return (
    <div className="space-y-1.5 pt-2">
      ...
      <p className="text-[11px] text-muted-foreground">{t("vms.wizard.securityGroupsHint")}</p>
    </div>
  )
}
...
function PublicIpOption({
  ...
  return (
    ...
    >
      ...
      <span className="block text-[11px] text-muted-foreground mt-0.5 pr-4">{description}</span>
    </motion.button>
  )
}
