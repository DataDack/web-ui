		...
		<div className="space-y-3">
			...
			>
				<SelectTrigger
					className={cn(
						"w-full",
						form.formState.errors.ssh_key_id && "border-destructive"
					)}
				>
...
function SshKeySheet({ open, onOpenChange, onSelect }: Readonly<SshKeySheetProps>) {
	...
	const options: { value: SshKeyMode; label: string; icon: typeof KeyRound; disabled?: boolean }[] = [
		{ value: "generate", label: t("sshKeys.wizard.generateOption"), icon: Sparkles, disabled: !canGenerate },
		...
		<Sheet open={open} onOpenChange={onOpenChange}>
			<SheetContent side="right" className="flex w-full max-w-130 flex-col gap-0 p-0">
				...
				<div className="flex flex-1 flex-col overflow-hidden">
					<div className="px-6">
						<Select value={mode} onValueChange={(v) => { setMode(v as SshKeyMode); }}>
					...
					<div className="flex-1 overflow-y-auto px-6 py-5">
						{mode === "generate" && (
							<SshKeyCreate method="generate" onCreated={(k) => { onSelect(k.id); }} />
						)}
						{mode === "import" && (
							<SshKeyCreate method="import" onCreated={(k) => { onSelect(k.id); }} />
						)}
					</div>
				</div>
			</SheetContent>
...
function SshKeyCreate({ method, onCreated }: Readonly<SshKeyCreateProps>) {
	...
	const autogenName = (): string | null => {
		...
		for (let attempt = 0; attempt < 10; attempt++) {
			...
			const candidates = [`key-${suffix}`, `key${suffix}`, `key${letters}`, `KEY${letters.toUpperCase()}`]
