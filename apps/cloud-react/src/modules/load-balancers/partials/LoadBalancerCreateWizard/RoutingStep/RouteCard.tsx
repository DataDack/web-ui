      ...
      <div className="grid gap-3 sm:grid-cols-2">
        ...
        <div className="space-y-1.5">
          ...
          {rowErrors?.allowed_cidrs ? (
            <p className="text-[11px] text-destructive">{rowErrors.allowed_cidrs.message}</p>
          ) : (
        ...
        <div className="space-y-1.5">
          ...
          >
            <SelectTrigger className="w-full">
              <SelectValue placeholder={t("loadBalancers.wizard.targetGroupPlaceholder")} />
            </SelectTrigger>
            <SelectContent>
          ...
          {rowErrors?.target_group_id && (
            <p className="text-[11px] text-destructive">{rowErrors.target_group_id.message}</p>
          )}
          <p className="text-[11px] text-muted-foreground">
        ...
        <>
          <div className="grid gap-3 sm:grid-cols-3">
            ...
            <div className="space-y-1.5">
              ...
              />
              {rowErrors?.tg_port && (
                <p className="text-[11px] text-destructive">{rowErrors.tg_port.message}</p>
              )}
            </div>
            <div className="space-y-1.5">
              <Label className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
              ...
              >
                ...
                <SelectContent>
                  ...
                  <SelectItem value="ip_hash">{t("targetGroups.algorithms.ip_hash")}</SelectItem>
                </SelectContent>
              </Select>
            </div>
          </div>
          ...
          <div className="mt-3">
            <HealthCheckFields form={form} index={index} isHTTP={l.protocol === "HTTP"} />
          </div>
          ...
          <div className="mt-3 space-y-1.5">
            ...
            <TargetPicker
              ...
              onPortChange={(instanceId, port) => {
                setValue(
                  `${path}.targets`,
                  l.targets.map((tgt) => (tgt.instance_id === instanceId ? { ...tgt, port } : tgt)),
                  { shouldValidate: true },
                )
              }}
            />
