...
export function RGField({
  ...
  if (isLoading) {
    listBody = <p className="px-2 py-1.5 text-xs text-muted-foreground">{t("common.loading")}</p>
  } else if (groups.length === 0) {
    listBody = (
      <p className="px-2 py-1.5 text-xs text-muted-foreground">{t("resourceGroups.field.none")}</p>
    )
  } else {
      ...
      return (
        ...
        >
          <span
            ...
            style={{
              background: rg.status === "active" ? "var(--success-pulse)" : "var(--bsc-outline)",
            }}
          />
    ...
    >
      ...
      <PopoverContent align="start" className="w-[--radix-popover-trigger-width] min-w-64 p-0">
        {mode === "create" ? (
          ...
          <div className="p-1">
            <p className="px-2 py-1.5 text-xs text-muted-foreground">{t("nav.rgSelector")}</p>
  ...
  return (
    <div className="space-y-3 p-4">
      <div className="space-y-0.5">
        <p className="text-sm font-medium text-foreground">{t("resourceGroups.field.create")}</p>
