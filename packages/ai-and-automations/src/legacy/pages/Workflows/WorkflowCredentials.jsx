...
import {
  Key, Pencil, Trash2, Plus, Loader2, Search, Unlink, ChevronDown,
      ...
      <CardContent className='p-4'>
        <div className='flex items-start gap-3'>
          ...
          <div
            ...
            style={isAccount ? { background: `${row.color}10`, borderColor: `${row.color}30` } : undefined}
          >
            {isAccount && RowIcon ? (
...
export default function WorkflowCredentials() {
  ...
  const featureStatus = maintenanceConfig.workflows === true ? 'maintenance' : (maintenanceConfig.workflows || 'off');
  ...
  const loading = accountsLoading || credsLoading;
      ...
      {loading ? (
        ...
        <div className='rounded-lg border border-dashed p-12 text-center'>
          ...
          <p className='text-[11px] text-muted-foreground mt-1'>
            {search ? 'Try a different search term' : 'Click "Add" to connect an account or create an API credential'}
          </p>
        </div>
      ) : (
