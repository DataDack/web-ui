      ...
      <PopoverContent align="end" sideOffset={8} className="w-[360px] p-5 space-y-5">
        ...
        <div className="space-y-1">
          ...
          <p className="text-xs text-muted-foreground leading-relaxed">
            Publish a new version to Lambda. Choose a version bump type and optionally describe what
            changed.
          </p>
        </div>
        ...
        <div className="space-y-2.5">
          <div className="flex items-center justify-between">
            ...
            <span className="font-mono text-xs text-muted-foreground tabular-nums">
              {formatVersion(current)} &rarr;{" "}
              <span className="text-foreground font-medium">{formatVersion(next)}</span>
            </span>
          </div>
        ...
        <div className="space-y-1.5">
          ...
          <Input
            ...
            onKeyDown={(e) => {
              if (e.key === "Enter" && !deploying) handleDeploy()
            }}
        ...
        {/* Deploy button */}
        <Button type="button" onClick={handleDeploy} disabled={deploying} className="w-full h-9">
...
export default function StudioToolbar({
  ...
  const brandIconUrl = getTransport().brandIconUrl
  ...
  return (
    <TooltipProvider delayDuration={300}>
      <div className="shrink-0">
        {/* Main bar */}
        <div
          className="h-14 flex items-center gap-3 px-4 backdrop-blur-xl border-b border-border"
          style={{ backgroundColor: "var(--workflow-panel-bg)" }}
        >
          ...
          <div className="flex items-center gap-2 shrink-0">
            <div className="w-8 h-8 rounded-xl bg-muted flex items-center justify-center overflow-hidden">
              {brandIconUrl ? (
                <img src={brandIconUrl} alt="" className="w-6 h-6 object-contain" />
              ) : (
                <span aria-hidden="true" className="text-sm font-bold text-[#D4AF37]">
                  D
                </span>
              )}
            </div>
            <div className="flex flex-col">
              ...
              <span className="text-[10px] text-muted-foreground leading-tight dark:text-muted-foreground/80">
                Workflow
              </span>
            </div>
          </div>
          ...
          {!isDeployed ? (
            <DeployPopover deploying={deploying} currentVersion={latestVersion} onDeploy={onDeploy}>
